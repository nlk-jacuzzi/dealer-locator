        ...
        <td id="main">
            ...
            for($i = 0; $i < 4; $i++){
                ...
                if(isset($data[$i]) && !empty($data[$i])){
                    ...
                    //echo $this->Image->resize('/files/dealer_imgs/'.$id.'/store/'.$data[$i]['Image']['path'], 304, 205, array('class' => 'dealerimg'));
                    echo '<img src="'.$this->Html->url($this->Image->resizedUrl('/files/dealer_imgs/'.$id.'/store/'.$data[$i]['Image']['path'], 304, 205, array('class' => 'dealerimg'))).'?'.rand(10000, 99999).'" />';
                ...
                if(isset($orig_data)){
                    ...
                    echo (empty($orig_data[$i]['Image']['path']) ? '' : '<img src="'.$this->Html->url($this->Image->resizedUrl('/files/dealer_imgs/'.(isset($copy_id) ? $copy_id : $orig_id).'/store/'.$orig_data[$i]['Image']['path'], 304, 205, array('class' => 'dealerimg'))).'?'.rand(10000, 99999).'" />').'</td></tr>';
                }
                echo '</td></tr>';
            }?>
            </table>
