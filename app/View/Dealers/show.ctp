...
Sunday: CLOSED<br />';
                        ...
                        $fields['about_title']['after'] = '<br /><font color="black"><a href="'.$this->Html->url('/files/JHT_Writing_Style_Guide_For_Dealers.pdf').'" target="_blank">Please click here for dealer writing guidelines</a>.</font>
                            </td>';
                        $fields['show_service_area']['after'] = '<br /><font color="black">Place a checkmark in this box to show a listing of nearby cities that are serviced by this location.</font>
                            </td>';
                        $fields['wet_test']['after'] = '<br /><font color="black">Place a checkmark in this box if the dealership has hot tubs available to wet test.</font>
                            </td>';
                            ...
                            if(($admin == 1 && (!isset($data['Dealer']['dealer_id']) || empty($data['Dealer']['dealer_id']))) || !isset($field['dealer_limit'])){
                                ...
                                <?php 
                                    ...
                                    if(!in_array($field_name, array('published', 'id', 'dealer_id', 'approval_ready')) && ((isset($presave_data) && isset($orig_dealer_data) && $orig_dealer_data['Dealer'][$field_name] != $presave_data['Dealer'][$field_name]) || (!isset($presave_data) && isset($orig_dealer_data) && $orig_dealer_data['Dealer'][$field_name] != $data['Dealer'][$field_name]))){
                                        ...
                                        }elseif($field_name == 'wet_test' || $field_name == 'show_service_area' || $field_name == 'j_500_banner'){
                                            $orig_value = ($orig_dealer_data['Dealer'][$field_name] == 1 ? 'Yes' : 'No');
                                        }else{
                        ...
                        <?php if(!empty($data['Dealer']['dealer_id'])){?>
                            <div class="form-bottom">
                                <p>
                                    ...
                                    <?php 
                                    $default_approval_text = 'Thank you. The edits you made to your Jacuzzi® Hot Tubs dealer page have been approved. You may see your changes by visiting your page online at '.$this->Html->url('/', true).'.

To request more updates please log in to your account here: '.$this->Html->url('/dealers/login', true).'

Thank you.';
                                    echo $this->Form->textarea('Message.approval_message', array('style' => 'width:514px', 'default' => $default_approval_text));?><br />
...
<script language="JavaScript" type="text/javascript">
    function changeSlug(cityInput){
        if(confirm('You have changed the city name.  Do you want the URL slug to change to match the new city?')){
            ...
            var slug = city.split(" ").join("-")+'-hot-tubs';
            $('DealerSlug').value = slug;
        }
    }
