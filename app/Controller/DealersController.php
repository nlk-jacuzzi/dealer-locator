 ...
 */
    ...
    function set_default_slugs(){
        ...
        foreach($dealers as $d){
            ...
            $d['Dealer']['slug'] = str_replace(' ', '-', strtolower($cityName)).'-hot-tubs';
            $slug = $d['Dealer']['slug'];
            $i = 2;
    ...
    function email_notify_approval_ready($id){
        ...
        $subject = 'Jacuzzi Dealer Approval Requested';
    ...
    function email_dealer_feedback($id, $feedback, $approved = 1)
    {
        ...
        if($approved == 1){
            $subject = 'Jacuzzi Dealer Page Updates Approved';
        }else{
    ...
    function email_notify_marketing($id, $is_new = true)
    {
        ...
        if ($is_new) {
            $subject = 'New Jacuzzi Dealer Added';    
        }else {
            $subject = 'Jacuzzi Dealer Deleted';
        }

        $data = $this->Dealer->find('first', array('recursive' => -1, 'conditions' => array("Dealer.id" => $id)));
        if (!empty($data)) {
            $message = '<html><head><title>' . $subject . '</title></head>';
            $message .= '<body><p>A Jacuzzi dealer has been '.($is_new ? 'added' : 'deleted').'.</p>';
