...
class LocatorsController extends AppController
{
    ...
    function beforeFilter($subDomain = null){
        ...
        switch ($subDomain){
            ...
            default:
                ...
                $lngTxt['noRetailers']    = 'Sorry, there are no Jacuzzi dealerships within ';
                $lngTxt['contact']        = 'Please contact a Jacuzzi&reg; Hot Tubs representative by phone at ';
            ...
            case 'fr':
                ...
                $lngTxt['toFind']         = 'Pour trouver le d&#233;taillant Jacuzzi le plus proche, veuillez s&#233;lectionner votre pays. Si vous vivez aux Etas-Unis ou au Canada, entrez &#233;galement votre code postal.';
                //$lngTxt['ifUS']         = 'Si vous vivez aux Etas-Unis ou au Canada, entrez ÃÂ©galement votre code postal.';
                $lngTxt['country']        = 'Pays:';
                $lngTxt['zipCode']        = 'Code postal:';
                $lngTxt['noRetailers']    = 'D&#233;sol&#233; mais il n&#39;y a pas de d&#233positaire Jacuzzi dans un rayon de ';
                $lngTxt['contact']        = 'Veuillez communiquer avec un repr&#233;sentant anglais Jacuzzi&reg; Hot Tubs par t&#233;l&#233;phone en csant le ';
                ...
                $lngTxt['custSupport']    = "CONTACTER JACUZZI";
        ...
        $this->set('jht_do_hreflang', $this->jht_do_hreflang());
        //pull all dealer cities in Quebec and Ontario for popular Canadian cities
    ...
    */
    ...
    function cities($stateInfo=null, $cityInfo=null, $extra_num=null){
        ...
        if(!empty($cityInfo) && !empty($stateInfo)) {
            ...
            //layout variables
            if($countryId == 3){
                $this->set('layoutTitle', 'Hot Tubs '.$cityName.': Find Accessories, Covers & Hot Tub Spas in '.$cityName.', '.strtoupper($stateAbbr).' | Jacuzzi Hot Tubs');
                $this->set('metaDesc', 'Find the world\'s most recognized brand of hot tubs and spas in '.$cityName.', '.$stateAbbr.'. Jacuzzi&reg; Hot Tubs dealers offer spa covers and accessories at stores in '.$cityName.', '.$stateAbbr);
            }else{
            ...
            $this->set('metaKeyword', $cityName.' Hot Tubs, '.$cityName.' Hot Tub Dealers, '.$cityName.' Hot Tub Stores, Hot Tubs in '.$cityName.', '.$stateAbbr.', Jacuzzi Dealers '.$cityName.', '.$stateAbbr);
        ...
        if(empty($dealer[0]['Image'])){
            $dealer[0]['Image'] = array(
                array(
                    'dealer_id' => 'default',
                    'path' => 'default_showroom_1.jpg'
                ),
                array(
                    'dealer_id' => 'default',
                    'path' => 'default_showroom_2.jpg'
                )
            );
        }
        if(empty($dealer[0]['Quote'])){
            $dealer[0]['Quote'] = array(
                array(
                    'quote' => 'The gentleman at the showroom explained everything to me and how each different hot tub had all these different features so it could really be customized to you and your family. I felt so much more empowered to make the right decision for us and our family.',
                    'name' => 'Kirsten A.'
                ),
                array(
                    'quote' => 'I would definitely recommend Jacuzzi brand to anybody looking for a hot tub. After having sat in that Jacuzzi on numerous occasions, I can tell you it’s an amazing experience.',
                    'name' => 'Kirsten A.'
                ),
                array(
                    'quote' => 'They talked about warranty. They talked about maintenance. All those types of features that you want to know before you make your final decision. The showroom experience I think made the difference in the whole process.',
                    'name' => 'Aleman Family'
                ),
                array(
                    'quote' => 'The quality of the tub as I compared them to other tubs really stood out for me. The fit and finish was very good as compared to the other tubs. And again, that just shows a sign of pride in the manufacturing process and a product that’s well done.',
                    'name' => 'Edward A.'
                )
            );
        }
        ...
        $showServiceArea = !empty($dealer[0]['Dealer']['show_service_area']);
        $hasLatitude = !empty($dealer[0]['Dealer']['latitude']);
        $hasLongitude = !empty($dealer[0]['Dealer']['longitude']);
        if ($showServiceArea && $hasLatitude && $hasLongitude) {
            // Get cities that are near enough
            $dealer_latitude = $dealer[0]['Dealer']['latitude'];
            $dealer_longitude = $dealer[0]['Dealer']['longitude'];
            $this->set('serviceArea', $this->nearby_cities_from_coordinates($dealer_latitude, $dealer_longitude));
        }
        else {
            // Not enough information to provide areas served
        }
    ...
    */
    ...
    function nearby_cities_from_coordinates( $latitude, $longitude, $max_distance_in_miles = 25, $limit = 25 )
    {
        //Only look at zips within about 100 km radius
        $min_lat = $latitude - 1;
        $max_lat = $latitude + 1;
        $min_lon = $longitude - (100/(111.320*cos($latitude)));
        $max_lon = $longitude + (100/(111.320*cos($latitude)));
        $query = <<<EOS
            Select
                min(
                    3959 
                ...
                city_population as TotalPopulation
            From
                postalcode_with_city_population
            Where
                latitude >= $min_lat AND latitude <= $max_lat AND longitude >= $min_lon AND longitude <= $max_lon
            Group by
                state_province,
                city
            Having
                Distance <= $max_distance_in_miles
            Order by
...
EOS;
        ...
        $serviceArea = $this->Zipcode->query($query);

        $result = array();
        //echo '<pre>'.htmlentities(print_r($serviceArea, TRUE)).'</pre>';
        foreach ($serviceArea as $location) {
            $result []= $location['postalcode_with_city_population']['city'];
        }
    ...
    private function jht_do_hreflang() {
        $cake_dir = $this->base . '/';
        $ca_url = 'http://www.jacuzzi.ca/hot-tubs';
        $us_url = 'http://www.jacuzzi.com/hot-tubs';
        $url = 'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . "{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}";
        $p = parse_url($url);
        $sub = explode('.', $p['host']);
        if(isset($sub[0]) && $sub[0] == 'beta'){
            $ca_url = 'http://beta.jacuzzi.ca/hot-tubs';
        }
	$the_end = '/dealer-locator/'. str_replace( $cake_dir , '', $p['path'] );
	return '<link rel="alternate" href="'. $us_url . $the_end .'" hreflang="en-us" /><link rel="alternate" href="'. $ca_url . $the_end .'" hreflang="en-ca" />' ."\n";
    }
}
?>
